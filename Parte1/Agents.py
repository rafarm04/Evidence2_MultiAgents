...
class SecurityDepartmentModel(ap.Model):
    ...
    def update_grid(self, dron_positions):
        ...
        """
        Detectar si hay objetos en las filas y columnas de las cámaras.
        Marcar los objetos como visitados ("V").
        """
        for cam in self.cams:
            # Obtener la posición de la cámara
            x, y = map(int, cam.onto_camera.ubication.split(","))
            
            # Detectar objetos en la fila de la cámara
            for i in range(self.grid_size):
                if (x, i) in self.objects and self.objects[(x, i)]["estado"] == "no visitado":
                    self.objects[(x, i)]["estado"] = "visitado"
                    print(f"Camera at ({x}, {y}) detected an object in row at ({x}, {i})")

            # Detectar objetos en la columna de la cámara
            for j in range(self.grid_size):
                if (j, y) in self.objects and self.objects[(j, y)]["estado"] == "no visitado":
                    self.objects[(j, y)]["estado"] = "visitado"
                    print(f"Camera at ({x}, {y}) detected an object in column at ({j}, {y})")
